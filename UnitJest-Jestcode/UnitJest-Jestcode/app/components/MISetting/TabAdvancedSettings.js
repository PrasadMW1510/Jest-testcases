...
import {
  ...
  TAB_ADVANCED_SETTINGS_GRADE_9_AND_ABOVE_TEXT,
...
class TabAdvancedSettings extends Component {
          ...
          immGrade.get('bandRanges').forEach((immBandRange, bandIndex) => {
            ...
            if (mutableImmSettingsOnScreen.getIn(['bandsEnabled', bandIndex])) {
              ...
              if (bandIndex === firstEnabledBandIndex) {
                ...
                clearedHigh = immBandRange.getIn(['defaultInfo', 'high']);
              }
  ...
  handleChangeBandName = ({ bandIndex, value }) => {
    const newImmSettingsOnScreen = this.state.immSettingsOnScreen.setIn(
      ['bandNames', bandIndex],
      value
    );
    this.setState({
  ...
  validateBandValues = ({
    ...
    immBandRanges.forEach((immBandRange, bandIndex) => {
      ...
      // (the user doesn't have any control over the final band's high value.)
      if (immBandsEnabled.get(bandIndex) && bandIndex !== numberOfBands - 1) {
        ...
        // get the value of the final band's high value (that's our cap)
        const finalHighValue = immBandRanges.getIn([numberOfBands - 1, 'high']);
        const isValid =
          (isNaN(value) && !invalidateBlanks) ||
          value === -999998 ||
          (value > highestValue + 1 && value < finalHighValue);
  ...
  handleChangeBandHighValue = ({ bandIndex, gradeNumber, value }) => {
    const newImmSettingsOnScreen = this.state.immSettingsOnScreen.setIn(
      ['grades', gradeNumber, 'bandRanges', bandIndex, 'high'],
      value
    );
    this.setState({
      immSettingsOnScreen: newImmSettingsOnScreen,
  ...
  handleChangeNumberOfBands = event => {
    ...
    this.setState({
      ...
      immSettingsOnScreen: newImmSettingsOnScreen.set('proficientBandIndex', proficientBandIndex),
      saveAttempted: false,
      tabHasNoUnsavedChanges: false,
    });
    this.props.handleIsolateTab();
  ...
  handleChangeProficientBand = newProficientBandIndex => {
    const immSettingsOnScreen = this.state.immSettingsOnScreen.set(
      'proficientBandIndex',
      newProficientBandIndex
    );
    this.setState({
      immSettingsOnScreen,
      tabHasNoUnsavedChanges: false,
  ...
  handleRestoreDefault = () => {
    ...
    // finally set the default proficient band
    newImmSettingsOnScreen = newImmSettingsOnScreen.set('proficientBandIndex', 3);
    this.setState({
      immInvalidHighs: this.getClearedInvalidHighs(),
      immSettingsOnScreen: newImmSettingsOnScreen,
      saveAttempted: false,
  ...
  saveSettings = () => {
    ...
    if (isAllValid) {
      ...
      if (isBandNameBlank) {
        errorMessage = 'Please enter a name for each Performance Level.';
        isAllValid = false;
      } else {
  ...
  shouldBandShowAsterisk = immBandReferenceInfo =>
    this.props.immProficiencyBandData.get('shouldShowAsterisks') &&
    (immBandReferenceInfo.get('defaultName') === 'Proficient' ||
      immBandReferenceInfo.get('defaultName') === 'Advanced');

  render() {
    const { immProficiencyBandData, selectedCohortType, transformScore } = this.props;
    ...
    return (
      <Fragment>
        ...
        {isEditable && (
          <div className="mi-settings__grade-9-and-above-text">
            {TAB_ADVANCED_SETTINGS_GRADE_9_AND_ABOVE_TEXT}
          </div>
        )}
        {immProficiencyBandData.get('shouldShowAsterisks') && (
          <div className="mi-settings__legend-text">* Indicates College and Career readiness</div>
        )}
        <form onSubmit={this.handleSubmit}>
          <ProficiencyBandPanel
            bandValueTranslator={transformScore}
            ...
            heading="Set Performance Level Names and Ranges"
            immInvalidHighs={immInvalidHighs}
            immProficiencyBandData={immSettingsOnScreen}
            isEditable={isEditable}
            labelNumberOfBands="Number of performance levels:"
            labelSelectProficientBand={'Select "proficient" level:'}
          ...
          {isEditable ? (
            ...
            <div className="mi-settings__only-button-container">

...
describe('validate', () => {
  ...
  describe('classes', () => {
    it('should return a classes error when there is no classes value', () => {
      values = fromJS({});
      errors = validate(values);

      expect(errors.classes).toEqual(getErrorMsg(161));
    });

    it('should return a classes error when classes size is 0', () => {
      values = fromJS({
        classes: {},
      });
      errors = validate(values);

...
describe('PrintBookLabel Saga', () => {
  ...
  describe('postDeactivateQuizRequest', () => {
    let err = null;
    const action = {
      tableData: [
        {
    ...
    beforeEach(() => {
      err = 'mock error';
      generator = Saga.postDeactivateQuizRequest(action);
    });

    it('postDeactivateQuizRequest', () => {
    ...
    it('getPostDeactivateRequestFailure', () => {
      expect(generator.next().value).toEqual(select(selectSelector));
      expect(generator.throw(err).value).toEqual(put(Actions.getPostDeactivateRequestFailure(err)));
      expect(generator.next().value).toEqual(put(hideModal()));
    });
  });
  describe('defaultSaga', () => {
    ...
    it('All calls are made', () => {
      expect(generator.next().value).toEqual(
        all([takeLatest(Constants.DEACTIVATE_QUIZ_LIST, Saga.postDeactivateQuizRequest)])
      );
    });
  });
});
